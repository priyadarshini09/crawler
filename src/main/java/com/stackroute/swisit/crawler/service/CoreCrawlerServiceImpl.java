...
@Service
public class CoreCrawlerServiceImpl implements CoreCrawlerService{
	...
	public void getPageLinks(String URL) {
        if (!links.contains(URL)) {
            try {
            	System.out.println("in page link sevice");
                Document document = Jsoup.connect(URL).get();
                Elements otherLinks = document.select("a[href^=\"http://www.mkyong.com/page/\"]");
                System.out.println("22222 in page link sevice");
    ...
    public void getArticles() {
        ...
        links.forEach(x -> {
            Document document;
            try {
            	System.out.println("in article sevice");
